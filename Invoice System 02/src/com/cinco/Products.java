...
public class Products {
	...
	public String toString() {
		if (type.compareTo('E') == 0)
			...
			if (consult.getConsult() == null)
				consult.setConsult(new People(true));

			return String
					.format(" Name: %s\t Code: %s\n Type: %s\n Consult: %s       Consult Code: %s\n HourlyFee: %f\n",
							this.name, this.code, getType(), consult
									.getConsult().getName(), consult
									.getConsultPersonCode(), consult
									.getHourlyFee());
		}
			...
			return String
					.format(" Name: %s\t Code: %s\n Type: %s\n ServiceFee: %f\n Annual Fee: %f\t Daily Fee: %f\n",
							this.name, this.code, getType(),
							lic.getServiceFee(), lic.getAnnualFee(),
							lic.getDailyFee());
